...
void CInterfaceSetDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	//DDX_Radio(pDX, IDC_BTN_CHANGE_RATE1, m_btnChangeRate);
}
...
void CInterfaceSetDlg::OnOK()
{
	...
	//清除原有数据
	memset(&m_interfaceSetData, 0, sizeof(m_interfaceSetData));
    ...
    case 1:
		...
		for(int i = 0; i < 8; ++i)
		{
			...
			//m_interfaceSetData.OptionName.push_back(str);//注意
			strcpy(m_interfaceSetData.OptionName[i], str);
		}
		break;
	}
...
{
	//m_staticBtnName->ShowWindow(index);
	//m_editBtnName->ShowWindow(index);
