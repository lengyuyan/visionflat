...
LRESULT CSEBtn::OnBtnMessage( WPARAM wParam, LPARAM lParam )
{
	...
	((CEdit *)GetDlgItem(IDC_EDIT_SEB))->SetSel(length,length); 
...
BOOL CSEBtn::PreTranslateMessage(MSG* pMsg)
{
	bool bRet = false;
	...
	{
		if (pMsg->hwnd == GetDlgItem(IDC_EDIT_SEB)->m_hWnd)
		{
			CString str;
			double dValue = 0.0;
			bool insert = false;
			...
			int length = str.GetLength();
			((CEdit *)GetDlgItem(IDC_EDIT_SEB))->SetSel(length,length); 
			UpdateItem(str, (BTN_ALGORITHM)0, insert);
	...
	return (bRet ? TRUE : CDialogEx::PreTranslateMessage(pMsg)) ;
}