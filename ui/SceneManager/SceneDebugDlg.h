...
public:
	...
	void SceneInitGrab(int SceneID,int ProcessType,bool sendflag); //重新封装初始拍照函数。
	...
	void ClickProcessFunc();//单击列表处理项  旧方法
	void ClickWay2Func(); //单击列表处理项    新方法
...
//#endif
	...
	double m_height;
	int m_iT; //老版本中控件相关的参数
	int m_index; //老版本中控件相关的参数
    int m_iCurSceneID; //当前场景
	int m_CurProcIndex;//当前场景中的当前处理索引
	int m_CurCamIndex;      // 当前相机索引
    int m_iCurIndex;   // 当前场景中的第几个图像处理项，不是处理项索引，也不是相机索引，它用来确定图像处理库的图像向量中的图形位置。
    ...
    void DrawControl();
	//
	RoiCombType m_CombType;   // 绘制ROI的组合类型
	int m_ProcProcess;        //
