...
BOOL  CBaslerGigeCamera::IsIniTrlFactory = 0;
int  CBaslerGigeCamera::CameraNum = 0;
...
//继承的抽象虚函数 模式：异步同步  策略:抓头部和尾部
 bool CBaslerGigeCamera::InitCam(char* CamerName)   //初始化相机
 { 
	 SetSynchroMode(AsynchroGrab);
	 OpenModeCam(m_CamIndex, AsynchroGrab,m_GrabStrategy);//在这里就打开相机了
...
BOOL CBaslerGigeCamera::OpenBaslerCam(int CamIndex,EGrabStrategy strategy)
{
	...
	try{
		...
		camera.Open();
		camera.StartGrabbing(strategy);  //因为只能调用一次,直接
		m_GrabStrategy = strategy;
	}
	catch(...)
	{
 ...
 EXTERN_C void WINAPI ReleaseCamera( CameraHANDLE  pCamera)
{
	...
	if (TRUE == CBaslerGigeCamera::IsIniTrlFactory && 0 == CBaslerGigeCamera::CameraNum)
	{
		PylonTerminate();
		CBaslerGigeCamera::IsIniTrlFactory = 0;		
	}
