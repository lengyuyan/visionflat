...
{
	strcpy(m_CamerName,camerName);//名字
	m_isInitFlag = false;
	m_isstopGrab = false;
...
MVS2GBECamera::~MVS2GBECamera()
{
 ...
 bool MVS2GBECamera::InitCam(char* CamerName) //初始化相机
{
	...
	int Index = 0;
	int i = 0;
	for (i = 0;i<EnumDeviceList.nDeviceNum;i++)
	{
		if (!strcmp((char*)EnumDeviceList.pDeviceInfo[i]->SpecialInfo.stGigEInfo.chSerialNumber,CamerName))
		{
			Index = i;
			break;
		}
	}
	if (i>=EnumDeviceList.nDeviceNum)
	{
		return false;
	}
	...
	//连接设备
	if( ret = MV_CC_OpenDevice(m_handle,MV_ACCESS_Control,0) )
		return false;
	else
		return true;
	//获取设备信息
	ret = MV_CC_GetDeviceInfo(m_handle,&m_DeviceInfo);
}
...
DWORD WINAPI MVS2GBECamera::ThreadGrading(LPVOID lpParamt)                  //采集图像的线程
{
	...
	//	unsigned char * pBufForDriver = (unsigned char *)malloc(nBufSize); //驱动指针

	    MV_FRAME_OUT_INFO_EX stImageInfo = {0};//图像信息
		...
		try
		{
			...
			//MV_CC_GetOneFrameTimeout(temp->m_handle,pBufForDriver,nBufSize,temp->pFrameInfo,1000);
			int nRet = MV_CC_GetOneFrameTimeout(temp->m_handle,temp->m_pBuffer,nBufSize,&stImageInfo,1000);
			if (nRet == MV_OK)
			{	
				temp->m_fcallback(temp->m_pGrabParent);
			}
		}
		catch (...)
		{
...
bool MVS2GBECamera::SingleSnap(bool bDisplay ) 	// 单帧采集 // 使用的是同步采集，不使用回调函数
{
	...
	//连接设备
	int ret = 0;
	/*int ret = MV_CC_OpenDevice(m_handle,MV_ACCESS_Control,0);*/
	//if( ret)
	//	return false;
	//设置采集模式为单帧
	//m_GrabStrategy=MV_ACQ_MODE_SINGLE;

	//if( ret = MV_CC_SetAcquisitionMode(m_handle,m_GrabStrategy) )
	//	return false;


	ret = MV_CC_SetEnumValue(m_handle, "AcquisitionMode", MV_ACQ_MODE_CONTINUOUS);
	if (MV_OK != ret)
	{
	...
	MVCC_INTVALUE stIntvalue = {0};
	ret = MV_CC_GetIntValue(m_handle, "PayloadSize", &stIntvalue);
	int nBufSizeForDriver = stIntvalue.nCurValue + 2048; //一帧数据大小+预留字节(用于SDK内部处理)
	unsigned char * pBufForDriver = (unsigned char *)malloc(nBufSizeForDriver);
	MV_FRAME_OUT_INFO_EX stImageInfo = {0};
	memset(&stImageInfo, 0, sizeof(MV_FRAME_OUT_INFO_EX));

	ret = MV_CC_StartGrabbing(m_handle);
	//ret = MV_CC_GetOneFrameTimeout(m_handle,pBufForDriver,nBufSizeForDriver,&stImageInfo,1000);
	ret = MV_CC_GetOneFrameTimeout(m_handle,m_pBuffer,nBufSizeForDriver,&stImageInfo,1000);
	return true;
}
...
bool MVS2GBECamera::StartSnap() 	// 连续采集
{
	...
	//	return true;
	m_isstopGrab = false;
	m_iscontinueGrab = true;
	m_hGrabEvent = CreateEvent(NULL,true,false,NULL);
...
{
	m_isstopGrab = true; //可以结束进程

	SetEvent(m_hGrabEvent); 
...
int MVS2GBECamera::GetGuidList(list<string> &guidList,string &type)          //获取GUID列表
{
	...
	for (i = 0;i<camerNum;i++)
	{
		...
		{
			cuuid = (char*)pstMVDevInfo->SpecialInfo.stGigEInfo.chSerialNumber;
			guidList.push_back(cuuid);
			printf("%s %x\n" , "nCurrentIp:" , pstMVDevInfo->SpecialInfo.stGigEInfo.nCurrentIp);                   //当前IP
			printf("%s %s\n\n" , "chUserDefinedName:" , pstMVDevInfo->SpecialInfo.stGigEInfo.chSerialNumber);     //用户定义名
		
		}
		else if (pstMVDevInfo->nTLayerType == MV_USB_DEVICE)
		{
			cuuid = (char*)pstMVDevInfo->SpecialInfo.stUsb3VInfo.chSerialNumber;
			guidList.push_back(cuuid);
